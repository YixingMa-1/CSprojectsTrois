...
#  Description: recursion training
...
# understand the recursiv e backtracking strategy in this problem,
...
def groupSum(start, nums, target):
    ...
    if start == len(nums):
        return False
    if groupSum(start + 1, nums, target - nums[start]) is True:
        return True
    else:
...
def groupSum6(start, nums, target):
    ...
    if groupSum6(start + 1, nums, target - nums[start]) is True:
        return True
    else:
...
def groupNoAdj(start, nums, target):
    ...
    if start >= len(nums):
        return False
    if groupNoAdj(start + 2, nums, target - nums[start]) is True:
        return True
    else:
...
def groupSum5(start, nums, target):
    ...
    if groupSum5(start + 1, nums, target - nums[start]) is True:
        return True
    else:
...
def groupSumClump(start, nums, target):
    ...
    for i in range(start + 1, len(nums)):
        if nums[start] == nums[i]:
            ...
            break
    if groupSumClump(start + m, nums, target - (m * nums[start])) is True:
        return True
    else:
        return groupSumClump(start + m, nums, target)
...
def splitArrayHelper( nums, index, result):
    if index == len(nums):
        return result == 0
    if splitArrayHelper(nums, index + 1, result + nums[index]) is True:
        return True
    else:
        return splitArrayHelper(nums, index + 1, result - nums[index])
# Given an array of ints, is it possible to divide the
...
def splitOdd10Helper(nums, index, group1, group2):
    if index == len(nums):
        return (group1 % 2 == 1 and group2 % 10 == 0) or \
               (group2 % 2 == 1 and group2 % 10 == 0)
    if splitOdd10Helper(nums, index + 1, group1 + nums[index], group2):
        return True
    return splitOdd10Helper(nums, index + 1, group1, group2 + nums[index])
...
def split53Helper(nums, index, result):
    if index == len(nums):
        return result == 0
    if nums[index] % 5 == 0:
        return split53Helper(nums, index + 1, result+ nums[index])
    if nums[index] % 3 == 0:
        return split53Helper(nums, index + 1, result - nums[index])
    if split53Helper(nums, index + 1, result + nums[index]) is True:
        return True
    return split53Helper(nums, index + 1, result - nums[index])
#######################################################################################################
#######################################################################################################
